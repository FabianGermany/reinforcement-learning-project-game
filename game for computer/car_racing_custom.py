...
class CarRacingCustom(gym.Env, EzPickle):
    metadata = {
    ...
    def __init__(self, verbose=1):
        ...
        self.all_discrete_actions =("ACTION_FULL_THROTTLE", #0
                                    "ACTION_SOFT_ACCELERATION", #1
                                    "ACTION_FULL_BRAKING", #2
                                    "ACTION_SOFT_BRAKING", #3
                                    "ACTION_TURN_LEFT_HARD", #4
                                    "ACTION_TURN_RIGHT_HARD", #5
                                    "ACTION_TURN_LEFT_SOFT", #6
                                    "ACTION_TURN_RIGHT_SOFT", #7
                                    "ACTION_ACCELERATE_AND_TURN_LEFT_SOFT", #8
                                    "ACTION_ACCELERATE_AND_TURN_RIGHT_SOFT", #9
                                    "ACTION_DECELERATE_AND_TURN_LEFT_SOFT", #10
                                    "ACTION_DECELERATE_AND_TURN_RIGHT_SOFT", #11
                                    "ACTION_DO_NOTHING") #12
                                    #all in all 13 actions
        ...
        #self.action_space = spaces.Discrete(12) #three values from 0 to 1
        self.action_space = spaces.Discrete(len(self.all_discrete_actions))
    ...
    def transform_action_space(self, action):
        # steer, gas, brake
        if action == 0: action = [0, 0.8, 0] #ACTION_FULL_THROTTLE
        elif action == 1: action = [0, 0.5, 0] #ACTION_SOFT_ACCELERATION
        elif action == 2: action = [0, 0, 0.8] #ACTION_FULL_BRAKING
        elif action == 3: action = [0, 0, 0.5] #ACTION_SOFT_BRAKING
        elif action == 4: action = [-0.8, 0, 0] #ACTION_TURN_LEFT_HARD
        elif action == 5: action = [0.8, 0, 0] #ACTION_TURN_RIGHT_HARD
        elif action == 6: action = [-0.5, 0, 0] #ACTION_TURN_LEFT_SOFT
        elif action == 7: action = [0.5, 0, 0] #ACTION_TURN_RIGHT_SOFT
        elif action == 8: action = [-0.5, 0.5, 0] #ACTION_ACCELERATE_AND_TURN_LEFT_SOFT
        elif action == 9: action = [0.5, 0.5, 0] #ACTION_ACCELERATE_AND_TURN_RIGHT_SOFT
    ...
    def step(self, action):
        ...
        action = self.transform_action_space(action) #discretize action space

        if action is not None:
            #print("Current Steering is {}." .format(action[0]))
            #print("Current Gas is {}." .format(action[1]))
            #print("Current Brake is {}." .format(action[2]))
    ...
    def render(self, mode="human"):
        ...
        t.disable()
        #self.render_indicators(WINDOW_W, WINDOW_H) #turn off info panel for image analysis
...
if __name__ == "__main__":
    ...
    #a = np.array([0.0, 0.0, 0.0]) #continuous values
    a = np.array([12]) #discrete values; start value is 12 (do nothing)
    ...
    def key_press(k, mod):
        ...
        if k == 0xFF0D:
            restart = True
        if k == key.LEFT:
            a[0] = 6 #no. 6 is soft left
        if k == key.RIGHT:
            a[0] = 7 #no. 7 is soft right
        if k == key.UP:
            a[0] = 1 #no. 1 is soft acceleration
        if k == key.DOWN:
            a[0] = 2 #no. is full brake

    def key_release(k, mod):
        if k == key.LEFT: # and a[0] == -1.0:
            a[0] = 12 #no. 12 is do nothing
